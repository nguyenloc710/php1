...
<?php
    $cates = new cates();
    if(isset($_GET['delid'])){
        $del = $cates->delete_cates($_GET['delid']);
    }if(isset($del)){
        echo $del;
    }
?>
...
<a href="userlist.php">Danh sách người dùng</a>
    ...
    <div class="content">
    <h3><a href="addcat.php">Thêm danh mục</a></h3>
...
<table border="1" style="border-collapse: collapse; width: 700px;">
    ...
    <?php
        $listcates = $cates->show_cates();
        if($listcates){
            while($row = $listcates->fetch_assoc()){
    ...
    <tr>
        ...
        <td>
        <a href="catedit.php?editid=<?php echo $row['catId'] ?>">Sửa</a>|
        <a onclick="return confirm('Ban co muon xoa nguoi dung nay');" href="?delid=<?php echo $row['catId'];?>">Xóa</a>
        </td>
    </tr>
    <?php
            }
